...
<!-- WISHLIST MOBILE -->
<ul
	class="shop_table cart wishlist_table wishlist_view responsive mobile <?php echo wp_kses_post($show_cb) ? 'with-checkbox' : ''; ?> <?php echo wp_kses_post($no_interactions) ? 'no-interactions' : ''; ?>"
	...
	if ( $wishlist && $wishlist->has_items() ) :
							...
							<?php if ( $show_variation || $show_dateadded || $show_price ) : ?>
								<table class="item-details-table">
									...
									<?php if ( $show_dateadded && $item->get_date_added() ) : ?>
										<tr>
											<td class="label">
												<?php esc_html_e( 'Added on:', 'perukar' ); ?>
											</td>
											<td class="value">
					...
					<div class="additional-info-wrapper">
						<?php if ( $show_quantity || $show_stock_status ) : ?>
							<table class="additional-info">
								<?php if ( $show_quantity ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Quantity:', 'perukar' ); ?>
										</td>
								...
								<?php if ( $show_stock_status ) : ?>
									<tr>
										<td class="label">
											<?php esc_html_e( 'Stock:', 'perukar' ); ?>
										</td>
										<td class="value">
											<?php echo 'out-of-stock' === $item->get_stock_status() ? '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of stock', 'perukar' ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'perukar' ) . '</span>'; ?>
										</td>
									</tr>
								<?php endif ?>
							</table>
						...
						<?php if ( $show_remove_product || $repeat_remove_button ) : ?>
							<div class="product-remove">
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'perukar' ) ) ); ?>"><i class="fa fa-trash"></i></a>
							</div>
						<?php endif; ?>
					</div>
				</li>
		...
		<p class="wishlist-empty">
			 ...
			 * @return string
			 */
			echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'perukar' ) ) );
			?>
		</p>
	<?php endif; ?>
