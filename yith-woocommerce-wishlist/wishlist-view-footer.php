...
<div class="yith_wcwl_wishlist_footer">
	<?php if ( $count && $show_cb ) : ?>
		...
		<div class="yith_wcwl_wishlist_bulk_action">
			<label for="bulk_actions">
				 ...
				 * @return string
				 */
				echo esc_html( apply_filters( 'yith_wcwl_wishlist_bulk_actions_label', __( 'Apply this action to all the selected items:', 'perukar' ) ) );
				?>
			</label>
			<select name="bulk_actions" id="bulk_actions">
				<option value="add_to_cart"><?php esc_html_e( 'Add to cart', 'perukar' ); ?></option>
				...
				<?php if ( $wishlist->current_user_can( 'remove_from_wishlist' ) ) : ?>
					<option value="delete">
						 ...
						 * @return string
						 */
						echo esc_html( apply_filters( 'yith_wcwl_wishlist_bulk_remove_label', __( 'Remove from wishlist', 'perukar' ) ) );
						?>
					</option>
				<?php endif; ?>
				...
				<?php if ( $available_multi_wishlist && count( $users_wishlists ) > 1 && $is_user_owner ) : ?>
					...
					foreach ( $users_wishlists as $wl ) :
						...
						<option value="<?php echo esc_attr( $wl->get_token() ); ?>">
							...
							echo esc_html( sprintf( __( 'Move to %s', 'perukar' ), $wl->get_formatted_name() ) );
							?>
						</option>
					<?php endforeach; ?>
				<?php endif; ?>
			</select>
			<input type="submit" name="apply_bulk_actions" value="<?php esc_attr_e( 'Apply', 'perukar' ); ?>"/>
		</div>
	<?php endif; ?>
	...
	<?php if ( $count && $show_update ) : ?>
		...
		<div class="yith_wcwl_wishlist_update">
			<input type="submit" name="update_wishlist" class="<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ); ?>" value="<?php esc_attr_e( 'Update', 'perukar' ); ?>"/>
		</div>
	<?php endif; ?>
	...
	<?php if ( $count && ( $show_ask_estimate_button || $enable_add_all_to_cart ) ) : ?>
		<div class="yith_wcwl_footer_additional_action">
			<?php if ( $count && $show_ask_estimate_button ) : ?>
				...
				<a href="<?php echo esc_url( ( $additional_info || ! is_user_logged_in() ) ? '#ask_an_estimate_popup' : $ask_estimate_url ); ?>" class="<?php echo esc_attr( $ask_an_estimate_classes ); ?> ask-an-estimate-button" <?php echo esc_attr( $additional_info || ! is_user_logged_in() ) ? 'data-rel="prettyPhoto[ask_an_estimate]"' : ''; ?> >
					<?php
					/**
			...
			<?php if ( $count && $enable_add_all_to_cart ) : ?>
				...
				<input type="submit" name="add_all_to_cart" value="<?php esc_attr_e( 'Add all to cart', 'perukar' ); ?>"/>
			<?php endif; ?>
		</div>
	<?php endif; ?>
</div>
