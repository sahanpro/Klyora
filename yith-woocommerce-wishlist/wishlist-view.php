...
<!-- WISHLIST TABLE -->
<table
	class="shop_table cart wishlist_table wishlist_view traditional responsive <?php echo wp_kses_post($no_interactions) ? 'no-interactions' : ''; ?> <?php echo wp_kses_post($enable_drag_n_drop) ? 'sortable' : ''; ?> "
	...
	<tr>
		...
		<th class="product-name">
			<span class="nobr">
				 ...
				 * @return string
				 */
				echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_name_heading', __( 'Product name', 'perukar' ), $wishlist ) );
				?>
			</span>
		</th>
		...
		<?php if ( $show_price || $show_price_variations ) : ?>
			...
			<th class="product-price">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_price_heading', __( 'Unit price', 'perukar' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_quantity ) : ?>
			...
			<th class="product-quantity">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', __( 'Quantity', 'perukar' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $show_stock_status ) : ?>
			...
			<th class="product-stock-status">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_stock_heading', __( 'Stock status', 'perukar' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
		...
		<?php if ( $enable_drag_n_drop ) : ?>
			...
			<th class="product-arrange">
				<span class="nobr">
					 ...
					 * @return string
					 */
					echo esc_html( apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', __( 'Arrange', 'perukar' ), $wishlist ) );
					?>
				</span>
			</th>
		<?php endif; ?>
	...
	if ( $wishlist && $wishlist->has_items() ) :
				...
				<tr id="yith-wcwl-row-<?php echo esc_attr( $item->get_product_id() ); ?>" data-row-id="<?php echo esc_attr( $item->get_product_id() ); ?>">
					...
					<?php if ( $show_remove_product ) : ?>
						<td class="product-remove">
							<div>
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove remove_from_wishlist" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'perukar' ) ) ); ?>">&times;</a>
							</div>
						</td>
					<?php endif; ?>
					...
					<?php if ( $show_stock_status ) : ?>
						<td class="product-stock-status">
							 ...
							 * @return string
							 */
							$stock_status_html = 'out-of-stock' === $item->get_stock_status() ? '<span class="wishlist-out-of-stock">' . esc_html( apply_filters( 'yith_wcwl_out_of_stock_label', __( 'Out of stock', 'perukar' ) ) ) . '</span>' : '<span class="wishlist-in-stock">' . esc_html( apply_filters( 'yith_wcwl_in_stock_label', __( 'In Stock', 'perukar' ) ) ) . '</span>';
					...
					<?php if ( $show_last_column ) : ?>
						<td class="product-add-to-cart">
							...
							if ( $show_dateadded && $item->get_date_added() ) :
								...
								echo '<span class="dateadded">' . esc_html( sprintf( __( 'Added on: %s', 'perukar' ), $item->get_date_added_formatted() ) ) . '</span>';
							endif;
							?>
							...
							<?php if ( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ) : ?>
								<?php if ( 'select' === $move_to_another_wishlist_type ) : ?>
									<select class="change-wishlist selectBox">
										<option value=""><?php esc_html_e( 'Move', 'perukar' ); ?></option>
										<?php
								...
								<?php else : ?>
									<a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
										 ...
										 * @return string
										 */
										echo esc_html( apply_filters( 'yith_wcwl_move_to_another_list_label', __( 'Move to another list &rsaquo;', 'perukar' ) ) );
										?>
									</a>
								<?php endif; ?>
							...
							if ( $repeat_remove_button ) :
								 ...
								 */
								?>
								<a href="<?php echo esc_url( $item->get_remove_url() ); ?>" class="remove_from_wishlist button" title="<?php echo esc_html( apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', __( 'Remove this product', 'perukar' ) ) ); ?>"><?php esc_html_e( 'Remove', 'perukar' ); ?></a>
							<?php endif; ?>
		...
		<tr>
			 ...
			 */
			?>
			<td colspan="<?php echo esc_attr( $column_count ); ?>" class="wishlist-empty"><?php echo esc_html( apply_filters( 'yith_wcwl_no_product_to_remove_message', __( 'No products added to the wishlist', 'perukar' ), $wishlist ) ); ?></td>
		</tr>
		<?php
	endif;
