...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email" action="<?php echo esc_url( $verify_url ); ?>">
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( 'We were unable to verify the email address you provided. Please try again.', 'perukar' ), 'error' );
	}
	?>
	<p>
		<?php
		printf(
			...
			esc_html__( 'To view this page, you must either %1$slogin%2$s or verify the email address associated with the order.', 'perukar' ),
	...
	<p class="form-row">
		<label for="email"><?php esc_html_e( 'Email address', 'perukar' ); ?>&nbsp;<span class="required">*</span></label>
