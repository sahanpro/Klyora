...
<div class="customer-history order-attribution-metabox">
	<h4>
		<?php
		esc_html_e( 'Total orders', 'perukar' );
		echo wp_kses_post(
			wc_help_tip(
				__( 'Total number of non-cancelled, non-failed orders for this customer, including the current one.', 'perukar' )
			)
		);
		?>
	</h4>
	...
	<h4>
		<?php
		esc_html_e( 'Total revenue', 'perukar' );
		echo wp_kses_post(
			wc_help_tip(
				__( "This is the Customer Lifetime Value, or the total amount you have earned from this customer's orders.", 'perukar' )
			)
		);
		?>
	</h4>
	...
	<h4><?php esc_html_e( 'Average order value', 'perukar' ); ?></h4>
