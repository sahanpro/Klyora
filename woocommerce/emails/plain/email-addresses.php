...
echo "\n" . esc_html( wc_strtoupper( esc_html__( 'Billing address', 'perukar' ) ) ) . "\n\n";
...
if ( ! wc_ship_to_billing_address_only() && $order->needs_shipping_address() ) {
	...
	if ( $shipping ) {
		echo "\n" . esc_html( wc_strtoupper( esc_html__( 'Shipping address', 'perukar' ) ) ) . "\n\n";
		echo preg_replace( '#<br\s*/?>#i', "\n", $shipping ) . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped

		if ( $order->get_shipping_phone() ) {
			echo wp_kses_post($order->get_shipping_phone()) . "\n"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
